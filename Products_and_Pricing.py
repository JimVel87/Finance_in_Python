...
def BSM_price(option, underlying_price, risk_free_interest_rate, volatility, dividend_yield):
    ...
    if option.option_style == 'American':
        ...
        d2 = d1 - volatility*math.sqrt(option.time_to_expiry);
        if option.option_type == 'call':
            return round(underlying_price*math.e**(-dividend_yield*option.time_to_expiry)*phi(d1) - option.strike_price*math.e**(-risk_free_interest_rate*option.time_to_expiry)*phi(d2),4);
        else:
            return round(option.strike_price*math.e**(-risk_free_interest_rate*option.time_to_expiry)*phi(-d2) - underlying_price*math.e**(-dividend_yield*option.time_to_expiry)*phi(-d1),4);
...
def BSM_warrant_price(warrant, underlying_price, risk_free_interest_rate, volatility, outstanding_shares, number_of_warrants, dividend_yield):
    ...
    if warrant.option_style == 'American':
        ...
        d1 = (math.log(underlying_price/warrant.strike_price, math.e) + (risk_free_interest_rate-dividend_yield+volatility**2/2)*warrant.time_to_expiry)/(volatility*math.sqrt(warrant.time_to_expiry));
        d2 = d1 - volatility*math.sqrt(warrant.time_to_expiry);
        if warrant.option_type == 'call':
            return round((underlying_price*math.e**(-dividend_yield*option.time_to_expiry)*phi(d1) - warrant.strike_price*math.e**(-risk_free_interest_rate*warrant.time_to_expiry)*phi(d2))*haircut,4);
        else:
            return round((warrant.strike_price*math.e**(-risk_free_interest_rate*warrant.time_to_expiry)*phi(-d2) - underlying_price*math.e**(-dividend_yield*option.time_to_expiry)*phi(-d1))*haircut,4);
...
def BSM_implied_volatility(option, option_price, underlying_price, risk_free_interest_rate, dividend_yield):
    """
    Given an option, its observed price and all other parameters, it goal-seeks the implied volatility. The result is given in array format.
    Starting estimate set to 10%.
    """
    return fsolve(BSM_for_fsolve, 0.1, args=(option_price, option, underlying_price, risk_free_interest_rate, dividend_yield));
...
def BSM_delta(option, underlying_price, risk_free_interest_rate, volatility, dividend_yield):
    if (option.option_style == 'American'):
        ...
        d1 = (math.log(underlying_price/option.strike_price, math.e) + (risk_free_interest_rate-dividend_yield+volatility**2/2)*option.time_to_expiry)/(volatility*math.sqrt(option.time_to_expiry));
        if option.option_type == 'call':
            return round(math.e**(-dividend_yield*option.time_to_expiry)*phi(d1),4);
        else:
            return round(math.e**(-dividend_yield*option.time_to_expiry)*(phi(d1)-1),4);
...
def BSM_gamma(option, underlying_price, risk_free_interest_rate, volatility, dividend_yield):
    if (option.option_style == 'American'):
        ...
        d1 = (math.log(underlying_price/option.strike_price, math.e) + (risk_free_interest_rate-dividend_yield+volatility**2/2)*option.time_to_expiry)/(volatility*math.sqrt(option.time_to_expiry));
        return round((math.e**(-d1**2/2)/math.sqrt(2*math.pi))/(underlying_price*volatility*math.sqrt(option.time_to_expiry)),4);
...
def BSM_vega(option, underlying_price, risk_free_interest_rate, volatility, dividend_yield):
    if (option.option_style == 'American'):
        ...
        d1 = (math.log(underlying_price/option.strike_price, math.e) + (risk_free_interest_rate-dividend_yield+volatility**2/2)*option.time_to_expiry)/(volatility*math.sqrt(option.time_to_expiry));
        return round(underlying_price*math.sqrt(option.time_to_expiry)*math.e**(-d1**2/2)/math.sqrt(2*math.pi),4);
...
def BSM_theta(option, underlying_price, risk_free_interest_rate, volatility, dividend_yield):
    if (option.option_style == 'American'):
        ...
        d1 = (math.log(underlying_price/option.strike_price, math.e) + (risk_free_interest_rate-dividend_yield+volatility**2/2)*option.time_to_expiry)/(volatility*math.sqrt(option.time_to_expiry));
        d2 = d1 - volatility*math.sqrt(option.time_to_expiry);
        part_1 = (-underlying_price*math.e**(-d1**2/2)/math.sqrt(2*math.pi)*volatility)/(2*math.sqrt(option.time_to_expiry));
        if option.option_type == 'call':
            return round(part_1 - risk_free_interest_rate*option.strike_price*math.e**(-risk_free_interest_rate*option.time_to_expiry)*phi(d2),4);
        else:
            return round(part_1 + risk_free_interest_rate*option.strike_price*math.e**(-risk_free_interest_rate*option.time_to_expiry)*phi(-d2),4);
...
def BSM_rho(option, underlying_price, risk_free_interest_rate, volatility, dividend_yield):
    if (option.option_style == 'American'):
        ...
        d2 = d1 - volatility*math.sqrt(option.time_to_expiry);
        if option.option_type == 'call':
            return round(option.strike_price*option.time_to_expiry*math.e**(-risk_free_interest_rate*option.time_to_expiry)*phi(d2),4);
        else:
            return round(-option.strike_price*option.time_to_expiry*math.e**(-risk_free_interest_rate*option.time_to_expiry)*phi(-d2),4);
